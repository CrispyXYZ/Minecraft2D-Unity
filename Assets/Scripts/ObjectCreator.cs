...
public class ObjectCreator
{
    ...
    public static void CreateObject(Vector3 position, string textureName){
        CreateObject(position, textureName, Guid.NewGuid().ToString());
    }

    public static void CreateObject(Vector3 position, string textureName, string name) {
        if(blocks.ContainsKey(position)) return;
        GameObject obj = new GameObject();
        obj.name = name;
        obj.AddComponent<SpriteRenderer>();
        obj.AddComponent<BoxCollider2D>();
        obj.GetComponent<SpriteRenderer>().sprite = Resources.Load(textureName, typeof(Sprite)) as Sprite;
